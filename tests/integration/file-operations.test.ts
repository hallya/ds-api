...
Deno.test("file-operations", async (t) => {
  await t.step("deleteFromSystem", async (t) => {
    ...
    await t.step(
      "validates path against downloadRootPath - path contains ..",
      async () => {
        const { path: tempDir, cleanup } = await createTempDir();
        try {
          const ds = new SynologyDS({
    ...
    await t.step(
      "throws error when downloadRootPath is not configured",
      async () => {
        const ds = new SynologyDS({
          baseUrl: config.nasUrl,
          username: "test",
        ...
        assert(results[0].reason instanceof Error);
        const error = results[0].reason as Error;
        assert(error.message.includes("Download root path is not configured"));
      }
    );

    await t.step("joins path with downloadRootPath correctly", async () => {
      const { path: tempDir, cleanup } = await createTempDir();
      try {
