...
export class SynologyDS {
  ...
  createFinalResult(tasksToPurge, totalSize, deleteResults) {
    ...
    return {
      message: `Purge completed: ${successfulDeletes.length
        } torrent(s) successfully deleted, ${failedDeletes.length} failure(s) (${this._formatSizeGB(totalSize)} GB)`,
      tasksToPurge,
      totalSize,
