...
export class SynologyDS {
  ...
  logDryRunDetails(tasksToPurge, totalSize) {
    ...
    this._logTasksList(tasksToPurge, "[DRY RUN] [Step 1/2]", "Simulated API deletion");

    const pathsToDelete = this._extractPathsFromTasks(tasksToPurge);
    const pathToTaskMap = this._createPathToTaskMap(tasksToPurge);
    this._logPathsList(pathsToDelete, pathToTaskMap, "[DRY RUN] [Step 2/2]", "Simulated system deletion", true);

    const totalSizeGB = this._formatSizeGB(totalSize);
  ...
  createDryRunResult(tasksToPurge, totalSize) {
    return {
      message: `[DRY RUN] Simulated purge: ${tasksToPurge.length
        } torrent(s) would be deleted (${this._formatSizeGB(totalSize)} GB)`,
      tasksToPurge,
      totalSize,
      dryRun: true,
    };
  ...
  async performApiDeletions(tasksToPurge) {
    ...
    this._logTasksList(tasksToPurge, "[Step 1/2]", "Starting API deletion");

    const deleteResults = await this.removeTasksByIds(ids, true);

    const successfulDeletes = this._filterSuccessfulDeletes(deleteResults);
    const failedDeletes = this._filterFailedDeletes(deleteResults);
    ...
    if (successfulDeletes.length > 0) {
      successfulDeletes.forEach((result) => {
        ...
        const sizeGB = task ? this._formatSizeGB(task.size) : 'N/A';
  ...
  async performSystemDeletions(deleteResults) {
    ...
    const successfulDeletes = Array.isArray(deleteResults)
      ? this._filterSuccessfulDeletes(deleteResults)
    ...
    const successfulTasks = successfulDeletes
      ...
      .filter(Boolean);
    const pathsToDelete = this._extractPathsFromTasks(successfulTasks);
    ...
    const pathToTaskMap = this._createPathToTaskMap(successfulTasks);
    this._logSystemDeletionPlan(pathsToDelete, pathToTaskMap);

    const systemDeleteResults = await this.deleteFromSystem(pathsToDelete);
    this._logSystemDeletionResults(systemDeleteResults, pathsToDelete, pathToTaskMap);
   ...
   * @private
   */
  _logSystemDeletionPlan(pathsToDelete, pathToTaskMap) {
    this._logPathsList(pathsToDelete, pathToTaskMap, "[Step 2/2]", "Starting system deletion");
  }
   ...
   * @private
   */
  _logSystemDeletionResults(systemDeleteResults, pathsToDelete, pathToTaskMap) {
  ...
  createFinalResult(tasksToPurge, totalSize, deleteResults) {
    ...
    const message = failedDeletes.length > 0
      ? `Purge completed with partial success: ${successfulDeletes.length} torrent(s) successfully deleted, ${failedDeletes.length} failure(s) (${this._formatSizeGB(totalSize)} GB)`
      : `Purge completed: ${successfulDeletes.length} torrent(s) successfully deleted (${this._formatSizeGB(totalSize)} GB)`;
  ...
  async removeTasksByTitles(titles) {
    ...
    const ids = titleList
      .map((title) => {
        ...
        if (task) {
          const sizeGB = this._formatSizeGB(task.size);
    ...
    const successfulDeletes = this._filterSuccessfulDeletes(results);
    const failedDeletes = this._filterFailedDeletes(results);
   ...
   * @private
   */
  _filterSuccessfulDeletes(deleteResults) {
   ...
   * @private
   */
  _filterFailedDeletes(deleteResults) {
    return deleteResults.filter((result) => result.error !== 0);
  }
   ...
   * @private
   */
  _logTasksList(tasks, prefix, stepLabel) {
   ...
   * @private
   */
  _logPathsList(paths, pathToTaskMap, prefix, stepLabel, includeFullPath = false) {
    if (paths.length === 0) {
