...
export class SynologyDS {
  ...
  logDryRunDetails(tasksToPurge, totalSize) {
    ...
    if (pathsToDelete.length > 0) {
      logger.info(`[DRY RUN] [Step 2/2] Simulated system deletion: ${pathsToDelete.length} file(s)/folder(s) would be deleted`);
      pathsToDelete.forEach((path, index) => {
        const task = tasksToPurge.find(t => t.additional?.detail?.destination === path);
        const taskTitle = task ? task.title : 'Unknown';
        const fullPath = `${this.path}${path}`;
        logger.info(`  → File ${index + 1}/${pathsToDelete.length}: "${fullPath}" (Task: "${taskTitle}")`);
      });
    } else {
      logger.info(`[DRY RUN] [Step 2/2] No system files would be deleted (no destination found)`);
    }
