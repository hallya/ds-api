...
export class SynologyDS {
   ...
   * @param {string} [options.basePath] - The base path for validation (defaults to path).
   * @param {Object} [options.config] - Configuration object to inject (for testing). If not provided, uses default config.
   */
  constructor(options = {}) {
  ...
  async deleteFromSystem(paths) {
    const promises = paths.map(async (pathItem) => {
      ...
      const fullPath = path.join(this.path, pathItem);
   ...
   * @returns {string} Size formatted as GB (decimal, 1000-based).
   * @private
   */
  #formatSizeGB(bytes) {
  ...
  #logPathsList(paths, pathToTaskMap, prefix, stepLabel, includeFullPath = false) {
    ...
    paths.forEach((pathItem, index) => {
      ...
      const taskTitle = task ? task.title : 'Unknown';
      const displayPath = includeFullPath ? path.join(this.path, pathItem) : pathItem;
